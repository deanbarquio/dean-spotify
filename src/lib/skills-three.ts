...
/** Subtle ambient lines behind browse row */
export function initSkillsBrowseCanvas(canvas: HTMLCanvasElement): SkillsThreeHandle | null {
  if (prefersReducedMotion()) return null;
  ...
  for (let i = 0; i < lineCount; i++) {
    const points = [];
    const y = (i - lineCount / 2) * 0.35;
    for (let x = -4; x <= 4; x += 0.5) {
      points.push(new THREE.Vector3(x, y + Math.sin(x * 0.8 + i) * 0.15, 0));
    }
  ...
  const animate = () => {
    ...
    resize();
    t += 0.006;
    group.position.x = Math.sin(t) * 0.15;
    group.children.forEach((line, i) => {
      line.position.y = Math.sin(t + (line.userData.offset as number)) * 0.08;
    });
    renderer.render(scene, camera);
  };
  resize();
